...
#             best_position = moved_position
#             best_fitness = fitness_moved

#     # Calculate the middle position
#     middle_position = np.mean(rotated_positions, axis=0)

#     return first_position, rotated_positions, best_position, middle_position
...
# first_position, rotated_positions, best_rotated_position, middle_position = explore_rotated_positions(current_position, circle_size)
...
# ax.scatter(*first_position, rosenbrock(*first_position), color='blue', label='Original Rotated Position')
...
# ax.scatter(*middle_position, rosenbrock(*middle_position), color='red', label='Middle Position')
...
#     best_position = normalized_position
...
#     return normalized_position, rotated_positions, best_position, middle_position

# # Example usage
# current_position = np.array([2.0, 3.0])

# # Explore rotated positions
# original_rotated_position, rotated_positions, best_rotated_position, middle_position = explore_rotated_positions(current_position)
...
#     global_best_position = particles[np.argmin([rosenbrock(*p) for p in particles])]
...
#             if rosenbrock(*best_pos) < rosenbrock(*personal_best_positions[i]):
#                 personal_best_positions[i] = best_pos
...
# # Example usage
# best_position, best_fitness = particle_swarm_optimization(iterations=100, swarm_size=20, search_range=5,
...
# ax.scatter(best_position[0], best_position[1], best_fitness, color='red', s=100, label='Optimized Position')
...
def eggshell(x, y, noise_strength=0):
    noise_shape = x.shape
...
def explore_rotated_positions(current_position, circle_size, angle_increment=120, max_distance=0.01):
    best_position = current_position
    best_fitness = eggshell(*current_position)
    ...
    for rotation_angle in range(0, 360, angle_increment):
        ...
        # Evaluate at the moved position
        fitness_moved = eggshell(*moved_position)
    ...
    middle_position = current_position
...
def particle_swarm_optimization(iterations, swarm_size, search_range, inertia_weight, cognitive_weight, social_weight):
    ...
    velocities = np.zeros_like(particles)
    personal_best_positions = particles.copy()
    global_best_position = particles[np.argmin([eggshell(*p) for p in particles])]
    global_best_fitness = eggshell(*global_best_position)
    best_fitness_values = []
    best_positions = []
    ...
    for iteration in range(iterations):
        for i in range(swarm_size):
            ...
            circle_size = 100 - (iteration / iterations) * (100 - 1)
            
            original_pos, rotated_pos, best_pos, middle_pos,  = explore_rotated_positions(particles[i], circle_size=circle_size)

            if eggshell(*best_pos) < eggshell(*personal_best_positions[i]):
                personal_best_positions[i] = best_pos

            if eggshell(*best_pos) < global_best_fitness:
                global_best_position = best_pos
                global_best_fitness = eggshell(*best_pos)
    ...
    return best_positions, best_fitness_values, cumulative_evaluations
...
def normal_particle_swarm_optimization(iterations, swarm_size, search_range, inertia_weight, cognitive_weight, social_weight):
    ...
    for iteration in range(iterations):
        for i in range(swarm_size):
            ...
            fitness_current = eggshell(*particles[i])
            fitness_personal_best = eggshell(*personal_best_positions[i])
...
best_positions_proposed, best_fitness_values_proposed, cumulative_evaluations_proposed = particle_swarm_optimization(iterations, swarm_size, search_range, inertia_weight, cognitive_weight, social_weight)

# Run the normal PSO
best_positions_normal, best_fitness_values_normal, cumulative_evaluations_normal = normal_particle_swarm_optimization(iterations, swarm_size, search_range, inertia_weight, cognitive_weight, social_weight)
...
ax2.plot_surface(X, Y, Z, cmap='viridis', alpha=0.3, label='Eggshell Function')

actual_optimum = np.array([1, 1])
actual_optimum_value = eggshell(*actual_optimum)
ax2.scatter(actual_optimum[0], actual_optimum[1], actual_optimum_value, color='green', s=100, label='Actual Optimum')

best_positions_proposed = np.array(best_positions_proposed)
ax2.plot(best_positions_proposed[:, 0], best_positions_proposed[:, 1], best_fitness_values_proposed, color='blue', label='Proposed PSO Position Progression')

ax2.set_xlabel('X')
ax2.set_ylabel('Y')
ax2.set_zlabel('Z')
ax2.set_title('Eggshell Function Optimization \n with Proposed PSO', y=1.02)
ax2.legend()
ax2.legend(loc='lower center', bbox_to_anchor=(0.5, -0.3))

ax3 = fig.add_subplot(133, projection='3d')
ax3.plot_surface(X, Y, Z, cmap='viridis', alpha=0.3, label='Eggshell Function')

actual_optimum = np.array([1, 1])
actual_optimum_value = eggshell(*actual_optimum)
ax3.scatter(actual_optimum[0], actual_optimum[1], actual_optimum_value, color='green', s=100, label='Actual Optimum')

best_positions_normal = np.array(best_positions_normal)
ax3.plot(best_positions_normal[:, 0], best_positions_normal[:, 1], best_fitness_values_normal, color='blue', label='Normal PSO Position Progression')

ax3.set_xlabel('X')
ax3.set_ylabel('Y')
ax3.set_zlabel('Z')
ax3.set_title('Eggshell Function Optimization \n with Normal PSO', y=1.02)
ax3.legend()
ax3.legend(loc='lower center', bbox_to_anchor=(0.5, -0.3))
