...
def rosenbrock(x, y, noise_strength=15000, a=1, b=100):
    noise_shape = x.shape
    noise = noise_strength * np.random.uniform(-1, 1, size=noise_shape)
    return (a - x)**2 + b * (y - x**2)**2 + noise
...
def particle_swarm_optimization(iterations, swarm_size, search_range, inertia_weight, cognitive_weight, social_weight):
    ...
    for iteration in range(iterations):
        for i in range(swarm_size):
            ...
            circle_size = 0.25 - (iteration / iterations) * (0.25 - 0)
        ...
        #     _, rotated_positions, _, _ = explore_rotated_positions_avg(random_particle, circle_size=0.05, search_range=search_range, angle_increment=angle_for_position_rotations)
        ...
        #     print(rotated_positions[:, 0],rotated_positions[:, 1])
        #     ax_example.scatter(random_particle[0], random_particle[1], color='blue', label='Particle Position')
        #     print(random_particle[0],random_particle[1])

        #     # Increase the number of minor ticks
        #     ax_example.xaxis.set_minor_locator(plt.MultipleLocator(0.005))  # Adjust the spacing as needed
        #     ax_example.yaxis.set_minor_locator(plt.MultipleLocator(0.005))  # Adjust the spacing as needed
        ...
        #     plt.subplots_adjust(left=0.16, bottom=0.12, top=0.93)
        #     ax_example.grid(True)
        #     ax_example.grid(True, which='minor', linestyle='--', linewidth=0.5)
        #     fig_example.set_size_inches(4, 4)
        #     plt.show()
